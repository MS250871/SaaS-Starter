...
async function resolvePreparedRows(params: {
  ...
  const identityOrFilters = [];

  if (emails.length > 0) {
    identityOrFilters.push({
      email: {
        in: emails,
      },
    });
  ...
  const authAccountOrFilters = [];

  if (emails.length > 0) {
    authAccountOrFilters.push({
  ...
  const [identities, authAccounts, customersByExternalId] = await Promise.all([
    identityOrFilters.length > 0
      ? prisma.identity.findMany({
          where: {
            OR: identityOrFilters,
          },
          select: {
            id: true,
            email: true,
      ...
      : Promise.resolve([]),
    authAccountOrFilters.length > 0
      ? prisma.authAccount.findMany({
          where: {
            OR: authAccountOrFilters,
          },
          select: {
            id: true,
            identityId: true,
  ...
  const identityByEmail = new Map(
    identities
      ...
      .map((identity) => [identity.email as string, identity]),
  );
  const identityByPhone = new Map(
    identities
      .filter((identity) => identity.phone)
      .map((identity) => [identity.phone as string, identity]),
  );
  const authAccountByEmail = new Map(
    authAccounts
      .filter((account) => account.type === AuthAccountType.EMAIL)
      .map((account) => [account.value.toLowerCase(), account]),
  );
  const authAccountByPhone = new Map(
    authAccounts
      .filter((account) => account.type === AuthAccountType.PHONE)
      .map((account) => [account.value, account]),
  );

  const existingIdentityIds = Array.from(identityById.keys());
  const existingCustomers = existingIdentityIds.length
