# Auth Actions Legacy Snapshot

Comparison-only snapshot captured from `git show HEAD:src/modules/auth/actions.ts` before the auth folder refactor.

Current extracted files to compare against:
- `src/modules/auth/actions/signup.action.ts`
- `src/modules/auth/actions/login.action.ts`
- `src/modules/auth/actions/verify.action.ts`
- `src/modules/auth/actions/resend.action.ts`
